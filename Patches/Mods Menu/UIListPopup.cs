    ...
    private void OnGUI()
    {
        ...
        foreach (Item card2 in this.cards)
        {
            ...
            else
            {
                ...
                if (image != null)
                {
                    GUI.DrawTexture(new Rect(this.fieldHeight + (this.fieldHeight * 0.21f), (num * this.fieldHeight) + ((this.fieldHeight - this.cardHeight) * 0.43f), this.cardWidth, this.cardHeight), image);
                }
                ...
                if (this.leftButtonEnabled && GUI.Button(rect9, string.Empty) && card2.selectable())
                {
